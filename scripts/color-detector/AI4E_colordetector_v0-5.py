...
path = "examples/mytest.png"  # path to image
...
if objectdetect == "Y":
    ...
    blob = cv.dnn.blobFromImage(image, 1.0 / 127.5, (320, 320), [127.5, 127.5, 127.5])
...
detected_colors, pixel_count = extcolors.extract_from_path(
    path_foreground, tolerance=tolerance
...
detected_colors_grouped = detected_colors.groupby(
    detected_colors["colorname"]
).aggregate(agg_functions)  # group colors
detected_colors_grouped["percentage"] = (
    detected_colors_grouped["pixels"] / detected_colors_grouped["pixels"].sum()
...
detected_colors_grouped = detected_colors_grouped[
    detected_colors_grouped.percentage > threshold
]
print(detected_colors_grouped)
