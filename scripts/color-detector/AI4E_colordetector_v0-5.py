...
if objectdetect == 'Y':
    ...
    foreground_img[np.where((mask2 == 0))] = np.array([0, 0, 0]).astype(
        'uint8')  # this allows you to change the background color after grabcut
...
detected_colors, pixel_count = extcolors.extract_from_path(
    path_foreground, tolerance=tolerance)  # no limit set, this will be done after grouping
...
detected_colors_grouped = detected_colors.groupby(
    detected_colors['colorname']).aggregate(agg_functions)  # group colors
detected_colors_grouped['percentage'] = (
    detected_colors_grouped['pixels'] / detected_colors_grouped['pixels'].sum()) * 100  # calculate percentage
